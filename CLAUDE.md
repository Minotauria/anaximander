...
- **v13 cut (2026-08-15)** — AI generation release: generate in map / in selection, 20-node caps, notes toggle + map-wide word budget, legend auto-fill (shapes/colours/edges) + floating resizable legend, draggable gen box (undimmed in-map); 18-finding audit hardening (shared AI-schema helpers, colour validation, stream lifecycle, frame-loop caching, `_lblMids` side table); history fixes (post-layout seed, restore-as-new-entry, live-state banking); ghost multi-select + per-label checkboxes (up to 10); menu restructure (drawer = rename + Home only, Export & share in tool panel, Import file splash tile, transient bottom-left save text, top-corner « / ≡ symmetry, first-word capitals).
